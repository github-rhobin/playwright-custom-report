...
test("inline test.step() + takeScreenshot()", {tag: ['@plain-style']},  async ({
  page,
}) => {
...
test("step() decorator + takeScreenshot() + page object class", {tag: ['@page-object-style']}, async ({ page }) => {
...
test("to-fail test + trace.zip", {tag: ['@plain-style']},  async ({
  page,
}) => {
  await test.step("Open Playwright site", async () => {
    await page.goto("https://playwright.dev/");
